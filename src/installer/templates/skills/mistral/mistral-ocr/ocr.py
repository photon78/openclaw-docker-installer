#!/usr/bin/env -S /home/hummer/.openclaw/workspace/skills/mistral-ocr/.venv/bin/python3
"""
...
Usage:
  ...
  python3 ocr.py --input <pfad> --extract-images --send --target <telegram_chat_id>
...
def ocr(input_path, pages=None, extract_images=False, output_dir=None,
    ...
    payload = {
        ...
        "document": doc,
        "include_image_base64": False,  # Rohdaten nicht nötig — wir croppen aus Original
    }
    if pages:
    ...
    # Bilder via Bounding-Box aus Original croppen
    if extract_images:
        ...
        for page_idx, page in enumerate(result.get("pages", [])):
            ...
            for img_meta in images:
                ...
                # Bounding-Box + Page-Dimensionen aus API-Response
                bbox = {
                ...
                if not is_url(input_path):
                    ...
                    print(f"⚠️ Crop aus URL nicht unterstützt — nutze --input mit lokalem Pfad.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--target", help="Telegram Chat-ID (required with --send)")
